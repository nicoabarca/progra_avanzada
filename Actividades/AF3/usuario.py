...
class Usuario:
    ...
    def ver_videos(self, nombre_lista):
        ...
        iterable_reproduccion = self.listas_reproduccion[nombre_lista]
        for pelicula in iterable_reproduccion:
            pelicula[0].reproducir()
            self.calificar_pelicula(pelicula)
            yield (pelicula[0].reproducir(), self.calificar_pelicula(pelicula))
