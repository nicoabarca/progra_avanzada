...
class Cliente:
    ...
    def conectar_al_servidor(self):
        ...
        print("El cliente se ha conectado exitosamente con el servidor.")

    def escuchar(self):
        # Completar
        self.thread = threading.Thread(target=self.thread_escuchar)
        self.thread.start()

    def thread_escuchar(self):
        # Completar
        while True:
            largo = self.socket_cliente.recv(4).decode()
            if not largo: break
            mensaje = ""
            bytes_leidos = 0
    ...
    def enviar(self, msg):
        # Completar
        msg_codificado = self.codificar_mensaje(msg)
        largo = msg_codificado[:4].decode()
        dict_msg = {"largo": largo, "mensaje": msg_codificado}
        self.socket_cliente.send(dict_msg, (self.host, self.port))
